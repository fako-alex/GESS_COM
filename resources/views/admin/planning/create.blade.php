...
<form method="post" action="{{ route('admin.planning.save') }}" class="mb-5 rounded-md border border-[#ebedf2] bg-white p-4 dark:border-[#191e3a] dark:bg-[#0e1726]">
        ...
        <div class="mb-5">
            <ol class="flex font-semibold text-primary dark:text-white-dark">
                ...
                <li class="bg-[#ebedf2] dark:bg-[#1b2e4b]">
                    <a
                        class="relative flex h-full items-center bg-primary p-1.5 text-white-light before:absolute before:inset-y-0 before:z-[1] before:m-auto before:h-0 before:w-0 before:border-[16px] before:border-l-[15px] before:border-r-0 before:border-t-transparent before:border-b-transparent before:border-l-primary ltr:pl-6 ltr:pr-2 ltr:before:-right-[15px] rtl:pr-6 rtl:pl-2 rtl:before:-left-[15px] rtl:before:rotate-180">Ajouter un planning</a>
                </li>
            </ol>
        </div>
    ...
    <div class="grid grid-cols-3 gap-5">
        <div class="col-span-1">
            ...
            <select id="service_id" name="service_id" class="w-full form-select" required>
                <option value="">-- Sélectionnez un service --</option>
                @foreach($services as $service)
                    <option value="{{ $service->id }}" {{ old('service_id') == $service->id ? 'selected' : '' }}>
                        {{ $service->name }} ( {{ $service->users_count }} Personnes )
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-span-1">
            ...
            <select id="user_id" name="user_id" class="w-full form-select" required>
                ...
                @if(old('user_id'))
                    ...
                    @endphp
                    @if($selectedUser)
                        <option value="{{ $selectedUser->id }}" selected>{{ $selectedUser->name }} {{ $selectedUser->prenom }}</option>
                    @endif
                @endif
            </select>
        ...
        <div class="col-span-1">
            ...
            <select id="planning_type" name="planning_type" class="w-full form-select" required>
                ...
                <option value="jour" {{ old('planning_type') == 'jour' ? 'selected' : '' }}>Jour</option>
                <option value="semaine" {{ old('planning_type') == 'semaine' ? 'selected' : '' }}>Semaine</option>
                <option value="mois" {{ old('planning_type') == 'mois' ? 'selected' : '' }}>Mois</option>
                <option value="annee" {{ old('planning_type') == 'annee' ? 'selected' : '' }}>Année</option>
            </select>
