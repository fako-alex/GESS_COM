        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <ol class="flex font-semibold text-primary dark:text-white-dark">
                    ...
                    <li class="bg-[#ebedf2] dark:bg-[#1b2e4b]">
                        <a
                            class="relative flex h-full items-center bg-primary p-1.5 text-white-light before:absolute before:inset-y-0 before:z-[1] before:m-auto before:h-0 before:w-0 before:border-[16px] before:border-l-[15px] before:border-r-0 before:border-t-transparent before:border-b-transparent before:border-l-primary ltr:pl-6 ltr:pr-2 ltr:before:-right-[15px] rtl:pr-6 rtl:pl-2 rtl:before:-left-[15px] rtl:before:rotate-180">Liste des plannings</a>
                    </li>
                </ol>
            </div>
            <div class="flex flex-col w-full gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                <div class="flex gap-3">
                    <div>
                        <a href="{{route('admin.planning.create')}}" class="btn btn-primary">
                        ...
                        <tbody>
                            @forelse($plannings as $planning)
                                <tr>
                                    ...
                                    <td>{{ $planning->user ? $planning->user->name . ' ' . $planning->user->first_name : 'Non défini' }}</td>
                                    <td>{{ $planning->planning_type }}</td>
                                    <td>{{ $planning->start_date }}</td>
                                    <td>{{ $planning->end_date }}</td>
                                    <td>{{ $planning->start_time }} H</td>
                                    <td>{{ $planning->end_time }} H</td>
                                    <td>{{ $planning->event_location }}</td>
                                    <td>
                                        @if($planning->status === 'confirme')
                                            <span class="px-3 py-1 rounded-full badge bg-success/20 text-success hover:top-0">Confirmé</span>
                                        @elseif($planning->status === 'en_attente')
                                            <span class="px-3 py-1 rounded-full badge bg-warning/20 text-warning hover:top-0">En cours</span>
                                        @elseif ($planning->status === 'annule')
                                            <span class="px-3 py-1 rounded-full badge bg-danger/20 text-danger hover:top-0">Annuler</span>
                                        @else
                                            <span class="text-gray-500">Non défini</span>
                                        @endif
                                    </td>
                                    <td>
                                        {{ $planning->addedByplanning ? $planning->addedByplanning->name . ' ' . $planning->addedByplanning->first_name : 'Non défini' }}
                                    </td>
                                    <td>{{ $planning->created_at ? \Carbon\Carbon::parse($planning->created_at)->format('d/m/Y') : ' ' }}</td>
                                    ...
                                    <td class="text-center">
                                        <ul class="flex items-center justify-center gap-2">
                                            <div x-data="{ open: false, selectedType: '' }">
                                                ...
                                                <li>
                                                    <a href="{{ route('admin.planning.update',['id'=> $planning->id]) }}"x-tooltip="Modifier">
                                            ...
                                            <div>
                                                <li>
                                                    <a onclick="confirmation(event)" href="{{ route('admin.planning.delete', $planning->id) }}"  x-tooltip="Supprimer">
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            ...
                            @empty
                                <tr class="text-center">
                                    <td colspan="3">Pas de planning</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
