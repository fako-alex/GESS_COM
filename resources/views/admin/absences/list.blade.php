                ...
                <div class="table-responsive">
                    <table id="myTable" class="table-striped table-hover">
                        <thead>
                            <tr>
                                <th>N°</th>
                                <th>Raisons d'absences</th>
                                <th>Date de début</th>
                                <th>Date de fin</th>
                                <th>Créer par</th>
                                <th>Description de l'absence</th>
                        ...
                        <tbody>
                            @forelse($absences as $absence)
                                <tr>
                                    ...
                                    <td>{{ $absence->name }}</td>
                                    <td>{{ $absence->start_date ? \Carbon\Carbon::parse($absence->start_date)->format('d/m/Y') : ' ' }}</td>
                                    <td>{{ $absence->end_date ? \Carbon\Carbon::parse($absence->end_date)->format('d/m/Y') : ' ' }}</td>
                                    <td>{{ $absence->addedByabsence ? $absence->addedByabsence->name . ' ' . $absence->addedByabsence->first_name : 'Non défini' }}</td>
                                    <td>{{ $absence->detail ?? 'Non défini'}}</td>
                                    <td>{{ $absence->created_at ? \Carbon\Carbon::parse($absence->created_at)->format('d/m/Y') : ' ' }}</td>
                                    <td class="text-center">
                                        <ul class="flex items-center justify-center gap-2">
                                            <div x-data="{ open: false, selectedType: '' }">
                                                <!-- Bouton Modifier avec modale -->
                                                <li>
                                                    <a href="{{ route('admin.absences.update',['id'=> $absence->id]) }}"x-tooltip="Modifier">
                                            ...
                                            <div>
                                                <li>
                                                    <a onclick="confirmation(event)" href="{{ route('admin.absences.delete', $absence->id) }}"  x-tooltip="Supprimer">
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
