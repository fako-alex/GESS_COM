...
<form method="post" action="{{ route('admin.users.update.submit', $user->id)}}" enctype="multipart/form-data"
    ...
    <div class="panel">
        ...
        <div class="flex justify-end">
            <!-- Ajout de mb-4 pour l'espacement -->
    ...
    <div class="flex flex-col sm:flex-row">
        <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-3">
            <div>
                ...
                <input id="name" name="name" value="{{ old('name', $user->name) }}" type="text" placeholder="Exp : FAKO"
                    class="form-input">
                @if($errors->has('name'))
            ...
            <div>
                ...
                <input id="first_name" name="first_name" value="{{ old('first_name', $user->first_name) }}" type="text"
                    placeholder="Exp : Alex" class="form-input">
            ...
            <div>
                ...
                <select id="gender" name="gender" class="form-select">
                    ...
                    <option value="Feminin" {{ old('gender', $user->gender) == 'Feminin' ? 'selected' : '' }}>Féminin
                    </option>
                    <option value="Masculin" {{ old('gender', $user->gender) == 'Masculin' ? 'selected' : '' }}>Masculin
                    </option>
                </select>
            ...
            <div>
                ...
                <input type="tel" id="phone" value="{{ old('first_name', $user->phone) }}" name="phone"
                    class="form-input" placeholder="(241) 77-22-71-07" maxlength="18" oninput="formatPhoneNumber(this)"
                    required>
                @if($errors->has('phone'))
            ...
            <div>
                ...
                <select id="service" name="service" class="form-input" required>
                    ...
                    <option value="{{ $service->id }}" {{ (old('service', $user->service_id) == $service->id) ?
                        'selected' : '' }}>
                        {{ $service->name }}
                    </option>
                    @endforeach
            ...
            <div>
                ...
                <input type="matricule" id="matricule" name="matricule" value="{{ old('matricule', $user->matricule) }}"
                    class="form-input" placeholder="Entrez votre matricule" required>
            ...
            <div>
                ...
                <input id="birth_date" value="{{ old('birth_date', $user->birth_date) }}" name="birth_date" type="date"
            ...
            <div>
                ...
                <input id="birth_place" value="{{ old('birth_place', $user->birth_place) }}" name="birth_place"
                    type="text" placeholder="Lieu de naissance" class="form-input">
            ...
            <div>
                ...
                <input id="country" value="{{ old('country', $user->country) }}" name="country" type="text"
                    placeholder="Lieu de naissance" class="form-input">
            ...
            <div>
                ...
                <input id="city" value="{{ old('city', $user->city) }}" name="city" type="text"
                    placeholder="Lieu de naissance" class="form-input">
                @if($errors->has('city'))
            ...
            <div>
                ...
                <input id="neighborhood" value="{{ old('neighborhood', $user->neighborhood) }}" name="neighborhood"
                    type="text" placeholder="Lieu de naissance" class="form-input" required>
            ...
            <div>
                ...
                <input id="hiring_date" value="{{ old('hiring_date', $user->hiring_date) }}" name="hiring_date"
            ...
            <div>
                ...
                <input id="departure_date" value="{{ old('departure_date', $user->departure_date) }}"
                    name="departure_date" type="date" class="form-input" onchange="toggleDepartureDiv()">
            ...
            <div id="departure_div"
                style="display: {{ $user->departure_date || old('departure_date') || request('departure_date') ? 'block' : 'none' }};">
                ...
                <select id="departure_id" name="departure_id" class="form-input" required>
                    ...
                    <option value="{{ $departure->id }}" {{ old('departure_id', $user->departure_id) == $departure->id ?
                        'selected' : '' }}>
                        {{ $departure->name }}
                    </option>
                    @endforeach
            ...
            <div>
                ...
                <select id="role" name="role" class="form-select" onchange="toggleFields()" required>
                    ...
                    <option value="Personnel" {{ old('role', $user->role) == 'Personnel' ? 'selected' : '' }}>Personnel
                    </option>
            ...
            <div id="password-field" style="display: none;">
                ...
                <div class="relative">
                    <input id="password" value="{{ old('password') }}" name="password" type="password"
                        class="form-input">
                    <button type="button" class="absolute transform -translate-y-1/2 right-2 top-1/2"
                        onclick="togglePasswordVisibility()">
                        <svg id="eye-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12c0 3.866-3.134 7-7 7s-7-3.134-7-7 3.134-7 7-7 7 3.134 7 7z"></path>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12l5 5M15 12l5-5"></path>
                        </svg>
                    </button>
                </div>
            ...
            <div>
                ...
                <select id="status" name="status" class="form-select text-white-dark" required>
                    ...
                    <option value="Inactif" {{ old('status', $user->status) == 'Inactif' ? 'selected' : '' }}>Inactif
                    </option>
                </select>
            ...
            <div>
                ...
                <input id="profile_picture" name="profile_picture" type="file" accept=".jpeg,.jpg,.png"
                ...
                <div id="currentImageContainer">
                    ...
                    <img src="{{ asset('documents/profil/users/' . $user->profile_picture) }}"
                        ...
                        class="w-full h-auto max-w-[150px] max-h-[100px] object-contain rounded-md shadow-md">
                    @else
            ...
            <div>
                <label for="absence_id">Raison d'absence <span class="text-danger">*</span></label>
                <select id="absence_id" name="absence_id" class="form-input">
                    <option value="">Choisissez une raison d'absence</option>
                    @foreach($absences as $absence)
                    <option value="{{ $absence->id }}" data-start="{{ $absence->start_date ?? '' }}"
                        data-end="{{ $absence->end_date ?? '' }}" {{ (old('absence_id', $user->absence_id) ==
                        $absence->id)
                        ? 'selected' : '' }}>
                        {{ $absence->name }}
                    </option>
                ...
                @if($errors->has('absence_id'))
                <span class="text-danger badge-outline-danger">{{ $errors->first('absence_id') }}</span>
                @endif
            </div>

            <!-- Conteneur des dates, caché par défaut -->
            <div id="dates-container" style="display: none;">
                <div>
                    <label for="start_date">Date de début d'absence</label>
                    <input id="start_date" name="start_date" value="{{ old('start_date', $user->start_date) }}"
                        type="date" class="form-input" readonly>
                </div>

                <div>
                    <label for="end_date">Date de fin d'absence</label>
                    <input id="end_date" name="end_date" value="{{ old('end_date', $user->end_date) }}" type="date"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let selectAbsence = document.getElementById('absence_id');
        let startDateInput = document.getElementById('start_date');
        let endDateInput = document.getElementById('end_date');
        let datesContainer = document.getElementById('dates-container');

        function updateDates() {
            let selectedOption = selectAbsence.options[selectAbsence.selectedIndex];
            ...
            if (startDate && endDate) {
                ...
                datesContainer.style.display = "block"; // Afficher les champs
            } else {
                startDateInput.value = "";
                endDateInput.value = "";
                datesContainer.style.display = "none"; // Cacher les champs
            }
        }

        selectAbsence.addEventListener('change', updateDates);
        updateDates(); // Initialiser l'affichage au chargement de la page
    });
</script>
