            ...
            <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-3">
                <div>
                    ...
                    <input id="name" name="name" value="{{ old('name', $user->name) }}" type="text"
                        placeholder="Exp : FAKO" class="form-input">
                    @if ($errors->has('name'))
                ...
                <div>
                    ...
                    <input id="first_name" name="first_name" value="{{ old('first_name', $user->first_name) }}"
                        type="text" placeholder="Exp : Alex" class="form-input">
                ...
                <div>
                    ...
                    <select id="gender" name="gender" class="form-select">
                        ...
                        <option value="Masculin" {{ old('gender', $user->gender) == 'Masculin' ? 'selected' : '' }}>
                            Masculin
                        </option>
                    </select>
                ...
                <div>
                    ...
                    <input type="tel" id="phone" value="{{ old('first_name', $user->phone) }}" name="phone"
                        class="form-input" placeholder="(241) 77-22-71-07" maxlength="18"
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}"
                        class="form-input" placeholder="Entrez votre email" required>
                    @if ($errors->has('email'))
                    ...
                    <select id="service" name="service" class="form-input" required>
                        ...
                        @foreach ($services as $service)
                            <option value="{{ $service->id }}"
                                {{ old('service', $user->service_id) == $service->id ? 'selected' : '' }}>
                                {{ $service->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="matricule" id="matricule" name="matricule"
                        value="{{ old('matricule', $user->matricule) }}" class="form-input"
                ...
                <div>
                    ...
                    <input id="birth_date" value="{{ old('birth_date', $user->birth_date) }}" name="birth_date"
                ...
                <div id="departure_div"
                    ...
                    <select id="departure_id" name="departure_id" class="form-input">
                        <option value="">Sélectionner un motif de départ</option>
                        @foreach ($departures as $departure)
                            <option value="{{ $departure->id }}"
                                {{ old('departure_id', $user->departure_id) == $departure->id ? 'selected' : '' }}>
                                {{ $departure->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="absence_id">Motif d'absence <span class="text-danger">*</span></label>
                    <select id="absence_id" name="absence_id" class="form-input">
                        <option value="">Choisissez une motif d'absence</option>
                        @foreach ($absences as $absence)
                            <option value="{{ $absence->id }}" data-start="{{ $absence->start_date ?? '' }}"
                                data-end="{{ $absence->end_date ?? '' }}"
                                {{ old('absence_id', $user->absence_id) == $absence->id ? 'selected' : '' }}>
                                {{ $absence->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        ...
        function formatPhoneNumber(input) {
            ...
            if (phoneNumber.length <= 3) {
                ...
                input.value =
                    `+(${phoneNumber.slice(0, 3)}) ${phoneNumber.slice(3, 5)}-${phoneNumber.slice(5, 7)}-${phoneNumber.slice(7)}`;
            } else if (phoneNumber.length <= 11) {
                input.value =
                    `+(${phoneNumber.slice(0, 3)}) ${phoneNumber.slice(3, 5)}-${phoneNumber.slice(5, 7)}-${phoneNumber.slice(7, 9)}-${phoneNumber.slice(9, 11)}`;
            } else {
                input.value =
                    `+(${phoneNumber.slice(0, 3)}) ${phoneNumber.slice(3, 5)}-${phoneNumber.slice(5, 7)}-${phoneNumber.slice(7, 9)}-${phoneNumber.slice(9, 11)}`;
            }
